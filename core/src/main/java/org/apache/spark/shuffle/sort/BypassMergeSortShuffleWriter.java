 ...
 */
/*
    bypassMergeSortShuffle 是一种带hash风格的基于sort的shuffle机制，
    为每个Reducer端的一个任务构建输出一个文件，并将各自的记录分别写入到各自对应的文件中，
    最后会基于各个分区的文件合并成一个大的输出文件。
    如果reducer任务数量非常大，这种shuffle机制效率是非常低的，因为它会为所有分区开启独立的序列化进程和文件流
...
final class BypassMergeSortShuffleWriter<K, V> extends ShuffleWriter<K, V> {
  ...
  // DiskBlockObjectWriter 数组，每个分区一个
  ...
  public void write(Iterator<Product2<K, V>> records) throws IOException {
    ...
    if (!records.hasNext()) {
      ...
      // 初始化索引文件的内容，此时对应各个分区的数据量或偏移量需要后移
    ...
    // 实例化 DiskBlockObjectWriter，为每个分区配置一个DiskBlockObjectWriter
    partitionWriters = new DiskBlockObjectWriter[numPartitions];
    /*
      todo : 通过循环方式写入数据到磁盘，此时会开启numPartitions 个DiskBlockObjectWriter
      因此分区数不宜设置过大，避免带来过重的内存开销
      DiskBlockObjectWriter 默认缓存大写为32K,比以前的100K 小很多，说明不适合打卡太多的DiskBlockObjectWriter实例
    */
    ...
    writeMetrics.incShuffleWriteTime(System.nanoTime() - openStartTime);
     ...
     // 遍历每条数据，根据partition将该记录交由DiskBlockObjectWriter 写入到各自对应的临时文件中
  ...
  /**
   * Concatenate all of te per-partition hfiles into a single combined file.
   *
