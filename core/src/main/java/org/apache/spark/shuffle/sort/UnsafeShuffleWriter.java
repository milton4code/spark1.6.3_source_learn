...
public class UnsafeShuffleWriter<K, V> extends ShuffleWriter<K, V> {
  ...
  public UnsafeShuffleWriter(
    ...
    // SortShuffleManager 为序列化模式试着最大输出分区数量
  ...
  public void write(scala.collection.Iterator<Product2<K, V>> records) throws IOException {
    ...
    // generic throwables.
    // 已经能成功追踪，并没有用一种标准的try catch 方式来处理，而是直接用throw Exception方式处理
  ...
  void closeAndWriteOutput() throws IOException {
   ...
   //  关闭外部排序器并获取全部spill信息
    ...
    // 通过shuffleBlockResolver解析器获取输出文件
    final File output = shuffleBlockResolver.getDataFile(shuffleId, mapId);

    // 在后续pill文件时先使用临时文件名，最终再重新命名为真正的输出文件
      // 在writeIndexFileAndCommit会多次使用到tmpFile
    ...
    try {
      ...
      // 将合并Spill后获取的分区及数据信息写入索引文件，并将临时文件重命名为真正的数据文件名
  ...
  void insertRecordIntoSorter(Product2<K, V> record) throws IOException {
    ...
    // 复位缓冲区
    // 内部使用ByteArrayOutputStream存放每条记录，容量为1M
    serBuffer.reset();
    ...
    // TODO 将记录插入到外部排序器中，serBuffer是一个字节数组
    //  内存数据偏移量为Platform.BYTE_ARRAY_OFFSE
    sorter.insertRecord(
   ...
   */
  /*
       将0到多个溢写文件合并，根据溢写文件数量和选定的压缩方式，选择最快的合并策略。
       返回包含合并文件中每个分区数据长度的数组
    */
  private long[] mergeSpills(SpillInfo[] spills, File outputFile) throws IOException {
    // 默认 spark.shuffle.compress = true
    ...
    try {
      if (spills.length == 0) {
          // 没有中间spill文件时，则创建一个空的文件
        new FileOutputStream(outputFile).close(); // Create an empty file
        return new long[partitioner.numPartitions()];
      } else if (spills.length == 1) {
          // 最后一个spills文件metrics已更新，不需要重复更新
        ...
        //由于有大量的spills需要合并，因此spills长度和shuflle写文件的时间都没有被统计。如果使用了慢合并路径
          // ，最终输出文件的大小和实际 的spills文件小小不一定相等。
          // 为了防范这个问题，所以会监控输出文件实际字节大小。
