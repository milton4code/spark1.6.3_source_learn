...
private[spark] class HashShuffleWriter[K, V](
  ...
  metrics.shuffleWriteMetrics = Some(writeMetrics)
   // 获取读写数据的 blockManager
  private val blockManager = SparkEnv.get.blockManager

  private val ser = Serializer.getSerializer(dep.serializer.getOrElse(null))
  // 从FileShuffleBlockResolver 获取指定的shuffleId和mapId对应分区个数所构建的数据块 ShuffleWriterGroup实例
  ...
  override def write(records: Iterator[Product2[K, V]]): Unit = {
    ...
    //TODO 根据partition获取每条数据对应的bucketId(即reduce序号)
    // 根据bucketId 获取ShuffleWriterGroup 实例，获取DiskBlockObjectWriter实例
    for (elem <- iter) {
