...
private[spark] class SortShuffleManager(conf: SparkConf) extends ShuffleManager with Logging {
  ...
  override def registerShuffle[K, V, C](
     ...
     // TODO 通过shouldBypassMergeSort 判断是否满足退回到Hash风格的shuffle条件
    if (SortShuffleWriter.shouldBypassMergeSort(SparkEnv.get.conf, dependency)) {
      ...
      //如果实际分区数量小于spark.shuffle.sort.bypassMergeThreshold partitions  设置的数量，同时不需要在Map端对数据进行聚合，这样直接写文件，最后合并。
...
//      Tungsten 模式 ，直接使用序列化模式时，可以减少内存开销和序列化反序列化方面的CPU，内存开销，因此更高效
  ...
  override def getWriter[K, V](
    ...
    // TODO  ShuffleHandle通过模式匹配，构建具体的writer
    handle match {
        // 采用序列化shuffle方式 ，其实就是Tungsten
        // 使用的数据路基与物理映射关系仍然为 IndexShuffleBlockResolver
