...
private[spark] class FileShuffleBlockResolver(conf: SparkConf)
   ...
   */
  /*
    由给定的map任务获取ShuffleWriterGroup实例，当writers成功关闭时，会注册为完成状态
   */
  def forMapTask(shuffleId: Int, mapId: Int, numReducers: Int, serializer: Serializer,
      writeMetrics: ShuffleWriteMetrics): ShuffleWriterGroup = {
    new ShuffleWriterGroup {
     //维护 指定的shuffleId 和numReducers 对应的map task 中的shuffle状态
      ...
      val writers: Array[DiskBlockObjectWriter] = {
        ...
        Array.tabulate[DiskBlockObjectWriter](numReducers) { bucketId => // bucketId 其实对应的时reduce id 或者说时partition id
          // 通过shuffleId, mapId, bucketId 组装blockId ，构建数据块逻辑标识
