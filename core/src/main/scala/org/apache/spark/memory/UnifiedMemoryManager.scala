...
/*
    TODO 同一内存管理
     将Storage和Execution是一种动态管理（软性），二者之间可以相互借用。
     通过spark.memory.fraction（默认0.75） 来配置共享内存，但系统会默认预留300M，所以真正可以分配的内存为totalHeap -300
    二者之间的内存通过spark.memory.storageFraction （默认0.5）设置。也就是各自真实申请到的内存为 0.75*0.5 = 0.375
    1）、Storage可以借用execution中空闲内存，但是在execution需要申请内存是会被回收（回收缓存内存，直到满足执行申请的内存）
    2）、Execution也可以storage空闲内存，但是由于实现的复杂性，借用的内存从来不会因为Storage的需要而被回收。
    TODO 复杂性原因 如果execution消耗了大部分的借用的storage内存，storage尝试申请内存可能会失败  ，在这种情况下，根据storage各自的剩余内存情况新的blocks数据会被 回收。
 */
...
object UnifiedMemoryManager {
  ...
  def apply(conf: SparkConf, numCores: Int): UnifiedMemoryManager = {
    ...
    new UnifiedMemoryManager(
      conf,
      maxMemory = maxMemory,
      // 通过配置 spark.memory.storageFraction 设置Execution和Storag共享内存的初始边界值，一半一半
      storageRegionSize =
  ...
  private def getMaxMemory(conf: SparkConf): Long = {
    ...
    // 当前最小的内存需要450M内存，不满足该条件会报错，退出
    if (systemMemory < minSystemMemory) {
      throw new IllegalArgumentException(s"System memory $systemMemory must " +
        s"be at least $minSystemMemory. Please use a larger heap size.")
    }
    val usableMemory = systemMemory - reservedMemory
    // Storage 和 Execution最大共享内存 usableMemory * 0.75
