...
/*
    MemoryManager 是一个抽象类，强制管理execution storage 之间内存共享。
    execution 内存在shuffle，join ，sort，aggregations 使用时会被调用
    storage 内存在缓存和 内部数据传输过程中是使用的内存。 一个JVM只有一个MemoryManager
  */
...
private[spark] abstract class MemoryManager(
  ...
  // storage部分的内存池初始化大小设置
  storageMemoryPool.incrementPoolSize(storageMemory)
  // ON_HEAP Execution 内存设置
  onHeapExecutionMemoryPool.incrementPoolSize(onHeapExecutionMemory)
  // 从配置文件spark.memory.offHeap.size 读取初始OFF_HEAP内存大小
