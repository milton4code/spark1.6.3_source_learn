...
private[spark] class BlockManager(
  ...
  /**
    ...
    */
  /*
  使用给定的appId初始化BlockManager。不是构造器中执行的，因为appId在BlockManager实例化时可能不无法知道。
  （特别对于driver端，只有向TaskScheduler注册后才能获取appId）

  该方法主要初始化BlockTransferService和ShuffleClient，注册BlockManagerMaster，并启动BlockManagerWorker 端点。
  如果做了本地化配置就会注册一个本地shuffle服务

   */
  def initialize(appId: String): Unit = {
    //初始化blockTransferService，用于在不同节点fetch 数据，传送数据
    ...
    //向BlockManagerMaster 在driver中注册BlockManager id.，传入blockManagerId，slaveEndpoint，将slaveEndpoint作为BlockManger中的RPC对象,
    //用于与blockManagerMaster进行通信。
    master.registerBlockManager(blockManagerId, maxMemory, slaveEndpoint)

    // Register Executors' configuration with the local shuffle service, if one should exist.
    //注册ExternalShuffleServer
  ...
  private def doGetLocal(blockId: BlockId, asBlockResult: Boolean): Option[Any] = {
     // 从blockInfo 这个TimeStampedHashMap 通过blockId查找是否有BlockInfo 信息
    val info = blockInfo.get(blockId).orNull
     // TODO 不为空 则说明从本地查找block数据成功
    if (info != null) {
      info.synchronized {
        ...
        //TODO 再次检查blockInfo 是否存在blockId,因为可能存在removeBlock 操作，这是一个 双保险操作
        ...
        // TODO 如果有另外的线程对该block数据进行操作，则一直等待，直到其他线程操作完成
        ...
        // TODO 如果使用Memeryc存储，就从内存中查找数据，并返回结果
        if (level.useMemory) {
        ...
        //TODO 使用OOF_HEAP存储 ，通过ExternalBlockManager查找数据
        if (level.useOffHeap) {
        ...
        //TODO 使用DISK级别存储数据，通过DiskStore查找数据
        if (level.useDisk) {
          ...
          if (!level.useMemory) { // 如果不能使用内存存储，则直接返回数据
            ...
            if (!level.deserialized || !asBlockResult) {
              ...
              // 如果block数据包含了memory序列化，或者在缓存到到内存中，则直接存放到内存中。

              // TODO 如果内存不够，则会OOM,所以将该操作放入ByteBuffer，并且是lazy操作
            ...
            if (!asBlockResult) {
              return Some(bytes)
            } else { // asBlockResult 为true
              // 反序列化数据
              val values = dataDeserialize(blockId, bytes)
              if (level.deserialized) {  // 已经被反序列化
  ...
  private def doGetRemote(blockId: BlockId, asBlockResult: Boolean): Option[Any] = {
    ...
    // TODO 调用BlockManagerMaster getLocations 获取blockLocations ,使用 Random.shuffle 避免每一次都从同一个远程节点取blockId对应的数据
    ...
    for (loc <- locations) {
      ...
      val data = try {
         // 调用BlockTransferService，异步fetch数据
        blockTransferService.fetchBlockSync(
          loc.host, loc.port, loc.executorId, blockId.toString).nioByteBuffer()
      } catch {
        case NonFatal(e) =>
          numFetchFailures += 1
          if (numFetchFailures == locations.size) { // 失败次数和locations size相同时输出警告信息
  ...
  // TODO 通过本地或者远程方式，从blockManger获取数据
  ...
  /**
    ...
    */
  //TODO 根据给定StorageLevel将数据存入一个节点中，若给定副本数则存储多份。
  private def doPut(
                     blockId: BlockId,
                     data: BlockValues,
                     level: StorageLevel,
  ...
  : Seq[(BlockId, BlockStatus)] = {
    ...
    // 校验 effectiveStorageLevel 数据合法性
    effectiveStorageLevel.foreach { level =>
      require(level != null && level.isValid, "Effective StorageLevel is null or invalid")
    }

    // Return value
    //TODO 定义一个ArrayBuffer 存放返回值数据
    val updatedBlocks = new ArrayBuffer[(BlockId, BlockStatus)]

    /* Remember the block's storage level so that we can correctly drop it to disk if it needs
     * to be dropped right after it got put into memory. Note, however, that other threads will
     * not be able to get() this block until we call markReady on its BlockInfo. */
    //TODO 根据给定的storageLevel 存放数据，如有必要，会将数据溢写到磁盘里。
    //TODO  值得注意的是，溢写到磁盘时其他的线程不能获取到溢写的数据，直到所有数据都溢写到磁盘中。
    val putBlockInfo = {
      val tinfo = new BlockInfo(level, tellMaster)
      // Do atomically !
      // 原子操作
      // 如果 blockInfo 中TimeStamped hashMapz没有blockId ,则加入此blockIdh和 tinfo.
      // putIfAbsent 返回值为key对应的更新此key之前的value.
      ...
      if (oldBlockOpt.isDefined) {
        ...
        //TODO 如果 oldBlockOpt 没有定义则返回 tinfo
        tinfo
      }
    }
    ...
    // ToDo 在数据本地化之前，先启动异步一个线程进行数据冗余备份。这有助于提高发送数据的速度，从而提高性能。
    val replicationFuture = data match {
      case b: ByteBufferValues if putLevel.replication > 1 =>
        ...
        Future {
          // This is a blocking action and should run in futureExecutionContext which is a cached
          // thread pool
          // TODO 在线程池中做数据备份操作，这是个阻塞操作。该操作时将blockId对应的bufferView复制到另一个节点上，在完成操作前都是阻塞的。
    ...
    putBlockInfo.synchronized {
      ...
      try {
        ...
        // blockStore - The type of storage to put these values into
         // TODO  returnValues  put操作返回的值
         // TODO  blockStore  put数据的stroeLevel
        val (returnValues, blockStore: BlockStore) = {
          if (putLevel.useMemory) {

            // Put it in memory first, even if it also has useDisk set to true;
            // We will drop it to disk later if the memory store can't hold it.
            // TODO 首先使用内存存储，即使设置了使用 disk存储，只有当内存不足才溢写到磁盘
            (true, memoryStore)
            ...
            // TODO 使用disck存储默认不返回bytes，除非使用了多个备份。
        ...
        // TODO 模式匹配put操作粗如的值，调用blockStore的方法
        val result = data match {
          case IteratorValues(iterator) =>  // 可迭代类型 则嗲用blockStore  putIterator 方法
            blockStore.putIterator(blockId, iterator, putLevel, returnValues)

          case ArrayValues(array) => // 数组类型，调用putArray 方法
            blockStore.putArray(blockId, array, putLevel, returnValues)
          case ByteBufferValues(bytes) => // ByteBuffer 类型
            bytes.rewind()
            blockStore.putBytes(blockId, bytes, putLevel)
        }
        // 获得结果大小
        size = result.size
        // TODO 对返回结果类型进行匹配，返回put操作后的result
        result.data match {
        ...
        // TODO 追踪那些从内存溢写到磁盘的数据

        //TODO 如果使用内存存储，遍历result结果中droppedBlocks，将溢出到磁盘中的block添加到updatedBlocks 数组中
        if (putLevel.useMemory) {
    ...
    // 如果putLevel 副本数大于 1，则进行异步复制操作
    if (putLevel.replication > 1) {
      data match {
        case ByteBufferValues(bytes) =>
          if (replicationFuture != null) {
            // 启动 replicationFuture 线程
