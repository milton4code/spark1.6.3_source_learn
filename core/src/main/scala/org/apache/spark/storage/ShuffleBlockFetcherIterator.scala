...
final class ShuffleBlockFetcherIterator(
   ...
   */
  // TODO 记录实际获取的block数量，这个数据量可能比传入的总数量小，因为过滤了block大小为0的块数据。
  ...
  private[this] def initialize(): Unit = {
    ...
    // 将本地数据或者远程数据进行分片
    /*
      读取本地数据和远程数据的方式是不一样的，因此先进行数据切分，
      切分数据时会考虑一次获取数据的大小（切分时会同时考虑并行数）封装请求
      最后会将剩余不足该带下的数据获取也封装为一个请求
     */
