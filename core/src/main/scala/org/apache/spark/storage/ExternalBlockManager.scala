 ...
 */
/*
使用外部BlockManager是必须要继承该类
调用该类必须由一个无参构造器，由ExternalBlockStore调用ini方法进行实例化。
该类所有的方法都由blockId作为主要参数，blockId在一个spark应用中必须唯一。
当时用外部块管理器时，应该避免任何多个spark应用的命名空间的冲突。
 */
private[spark] abstract class ExternalBlockManager {
  ...
  def putValues(blockId: BlockId, values: Iterator[_]): Unit = {
    // 调用blockManager 将block数据序列化为byteBuffer。
