...
// 存储在内存中，将block数据反序列为Java对象的数组或者是字节缓冲对象。
private[spark] class MemoryStore(blockManager: BlockManager, memoryManager: MemoryManager)
  ...
  override def getValues(blockId: BlockId): Option[Iterator[Any]] = {
   // 通过 entries 取出对应的 MemoryManager信息
    val entry = entries.synchronized {
      entries.get(blockId)
    }
    if (entry == null) {
      None
    } else if (entry.deserialized) { // 判断取出的entries 是否序列化，序列化，则返回 Array迭代器
      Some(entry.value.asInstanceOf[Array[Any]].iterator)
    } else { // 没有被序列化，取出entry ByteBuffer  副本，然后再将其序列化
   ...
   */
  /*
    如果内存充足，则将数据做put操作。数据要么是一个序列化的数组对象或者是ByteBuffer。
    数据的创建是个lazy的，如果没有足够的内存或者磁盘空间，则value不会被创建以免OOM
   */
  private def tryToPut(
      blockId: BlockId,
      value: () => Any,
    ...
    memoryManager.synchronized {
      ...
      if (enoughMemory) {
        ...
        // 为block 申请到足够的内存后，做put操作
        val entry = new MemoryEntry(value(), size, deserialized)
        entries.synchronized {
          // 将block数据存放到LinkedHashMap中
        ...
        lazy val data = if (deserialized) {
          Left(value().asInstanceOf[Array[Any]]) // deserialized 为true ,将values转为Aray数组
        } else { // deserialized 为false ,将values转为ByteBuffer实例
