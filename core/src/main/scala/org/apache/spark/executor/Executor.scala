...
private[spark] class Executor(
  ...
  //在CoarseGrainedExecutorBackend中实例化Eexecutor,isLocal设置为false,即Executor中的isLcal始终为false
  if (!isLocal) {
    //在metricsSystem注册资源
    env.metricsSystem.registerSource(executorSource)
    //调用blockManager的initialize方法完成向BlockManager注册，完成Executor中BlockManager向driver端的BlockManager注册。
  ...
  def launchTask(
    ...
    val tr = new TaskRunner(context, taskId = taskId, attemptNumber = attemptNumber, taskName,
      serializedTask)
    //将创建的TaskRunner 放入到runningTasks队列中（map）
  ...
  class TaskRunner(
    ...
    override def run(): Unit = {
      ...
      // ExecutorBackend的statusUpdate方法，向driver发送Task的Running状态
