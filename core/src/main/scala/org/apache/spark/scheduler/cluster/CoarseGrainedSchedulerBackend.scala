  ...
  class DriverEndpoint(override val rpcEnv: RpcEnv, sparkProperties: Seq[(String, String)])
    ...
    override def receiveAndReply(context: RpcCallContext): PartialFunction[Any, Unit] = {
      ...
      case RegisterExecutor(executorId, executorRef, hostPort, cores, logUrls) =>
        //检查是否包含executorId，如果包含，返回RegisterExecutorFailed，（不需要重复注册）
        if (executorDataMap.contains(executorId)) {
          context.reply(RegisterExecutorFailed("Duplicate executor ID: " + executorId))
        } else {
          // If the executor's rpc env is not listening for incoming connections, `hostPort`
          // will be null, and the client connection should be used to contact the executor.
          //如果executorRef.address 地址不为空，则取出executorRef 地址作为executor 的地址，
          // 否则使用sender的Address作为executor的address
         ...
         //在addressToExecutorId map中加入executorAddress  executorId映射关系
          addressToExecutorId(executorAddress) = executorId
         //增加 cores 个数
          totalCoreCount.addAndGet(cores)
          //totalRegisteredExecutors 加1
          totalRegisteredExecutors.addAndGet(1)
          val data = new ExecutorData(executorRef, executorRef.address, executorAddress.host, cores, cores, logUrls)
          ...
          //向事件总数中发送SparkListenerExecutorAdded 消息
          listenerBus.post(
