...
private[spark] class SparkDeploySchedulerBackend(
  ...
  override def start() {
    ...
    //args 会在ExecutorRunner启动CoarseGrainedExecutorBackend 时被替换为真实值
    val args = Seq(
      "--driver-url", driverUrl,
      "--executor-id", "{{EXECUTOR_ID}}", //EXECUTOR_ID 在构建该参数时，EXECUTOR还木有构建好
      "--hostname", "{{HOSTNAME}}",
      "--cores", "{{CORES}}",
      "--app-id", "{{APP_ID}}",
    ...
    //显时指定org.apache.spark.executor.CoarseGrainedExecutorBackend 启动类
    val command = Command("org.apache.spark.executor.CoarseGrainedExecutorBackend",
      args, sc.executorEnvs, classPathEntries ++ testingClassPath, libraryPathEntries, javaOpts)
    val appUIAddress = sc.ui.map(_.appUIAddress).getOrElse("")
    val coresPerExecutor = conf.getOption("spark.executor.cores").map(_.toInt)
    //使用conmmond 等信息创建ApplicationDescription 描述信息
