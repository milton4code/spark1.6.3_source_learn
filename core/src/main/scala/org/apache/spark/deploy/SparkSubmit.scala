...
private[deploy] object SparkSubmitAction extends Enumeration {
  ...
  val SUBMIT, KILL, pREQUEST_STATUS = Value
}
...
object SparkSubmit {
  ...
  def main(args: Array[String]): Unit = {
    // 由启动main函数传入参数构建SparkSubmitArguments
    ...
    appArgs.action match {
      ...
      case SparkSubmitAction.pREQUEST_STATUS => requestStatus(appArgs)
    }
  }
  ...
  /**
    * 将参数提交到应用程序中
    * 分两步走
    * 第一步：获取根据子程序classPath，系统属性、以及子进程参数
    * 第二步：使用上述参数，然后调用子main方法
    *
    */
   ...
   */
  /*
   应用程序提交环境准备工作，返回一个4元元组
   1、子进程参数 2、子进程classPath 3、系统环境变量 4、子进程的main方法入口
   */
  private[deploy] def prepareSubmitEnvironment(args: SparkSubmitArguments)
    ...
    if (args.isStandaloneCluster) {
      //使用 org.apache.spark.deploy.rest.RestSubmissionClient 模式
      if (args.useRest) {
  ...
  private def runMain(
    ...
    Thread.currentThread.setContextClassLoader(loader) //获取classLoader

    for (jar <- childClasspath) {
      // 使用类加载器将jar依赖加入到classPath中
      addJarToClasspath(jar, loader)
    }

    for ((key, value) <- sysProps) { //将sysProps加入到System全局变量中
    ...
    try {
      mainClass = Utils.classForName(childMainClass) //获取启动mainClass
    } catch {
    ...
    try {
      //通过反射执行main方法，并将childArgs作为参数传入main方法
