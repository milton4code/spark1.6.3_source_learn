...
private[deploy] class Master(
  ...
  override def receiveAndReply(context: RpcCallContext): PartialFunction[Any, Unit] = {
    ...
    case RequestSubmitDriver(description) => {
      //如果状态不为ALIVE，则向Clinet返回 SubmitDriverResponse消息
      if (state != RecoveryState.ALIVE) {
        ...
        //向ClientEndPoint回复SubmitDriverResponse消息
  ...
  private def allocateWorkerResourceToExecutors(
    ...
    //如果executor指定了个数，则将work中的资源/executor个数，否则把worker中所有资源分配到一个executor中
  ...
  private def schedule(): Unit =
    ...
    //随机打乱worker 次序，用于driver在集群中随机加载，负载均衡，所以，driver在executor中时随机启动的。
  ...
  private def launchExecutor(worker: WorkerInfo, exec: ExecutorDesc): Unit = {
    ...
    //exec 是 executor 的描述对象
    //向WorkerInfo添加exec
    worker.addExecutor(exec)
    //向worker发送launchExecutor信息，加载executor
    worker.endpoint.send(LaunchExecutor(masterUrl,
      exec.application.id, exec.id, exec.application.desc, exec.cores, exec.memory))
    //向driver端发送ExecutorAdded信息
