...
private[deploy] class ExecutorRunner(
  ...
  private def fetchAndRunExecutor() {
    try {
      // Launch the process
      //使用CommandUtils创建ProcessBuilder
      ...
      //在进程环境变量加入 SPARK_EXECUTOR_DIRS 进程工作目录
      ...
      //Executor进程退出后向worker发送ExecutorStateChanged消息，worker收到该消息后回收分配给该executor的资源（cpu core，memory）
